...
def _normalize_contract(raw: dict) -> GoalContract:
    ...
    reqs = [Requirement(key=item.get("id") or item.get("key") or "requirement", description=item.get("text") or item.get("description") or "", required=True) for item in raw.get("success_requirements", [])]
...
@app.get("/health")
def health():
    return {"status":"ok","product":"Future Call AI","epistemic_rule":"CALL_COMPLETED != TASK_COMPLETED","telephony":readiness().__dict__,"call_e":calle_readiness().__dict__}
...
def _ledger(consent: dict) -> ConsentLedger:
    return ConsentLedger(approved=consent["approved"],revoked=False,recipient=consent["recipient"],purpose=consent["purpose"],allowed_data=consent["allowed_data"],forbidden_actions=consent["forbidden_actions"],hard_constraints=consent["constraints"])
...
def simulate_call(req: SimulateCall):
    ...
    evidence=_classify_transcript(contract,req.transcript); result=verify_goal(contract,evidence,call_completed=True)
...
def calle_live_call(req: LiveCalleRequest):
    ...
    completed=provider.get("status")=="completed"; evidence=evidence_from_calle(provider,contract.success_conditions); result=verify_goal(contract,evidence,call_completed=completed)
    return {"mode":"LIVE_CALL_E","guardian":"ENFORCED","provider_state":provider.get("status"),"provider_task_completed":provider.get("task_completed"),"goal_state":result.state.value,"verified":result.verified,"missing":result.missing,"contradicted":result.contradicted,"reason":result.reason,"provider_evidence":provider.get("evidence",[]),"call_id":provider.get("id")}
