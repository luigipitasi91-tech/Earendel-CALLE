...
function App(){
 const [step,setStep]=useState(0),[consent,setConsent]=useState(false),[scenario,setScenario]=useState("verified"),[rating,setRating]=useState(0),[mode,setMode]=useState("simulated"),[phone,setPhone]=useState("");
 ...
 async function authorizeAndRun(){
  ...
  if(!calle.configured){setError("CALL-E is not configured on the server.");return}
  if(!/^\+[1-9]\d{7,14}$/.test(phone)){setError("Enter the destination in E.164 format, for example +447700900123.");return}
  setBusy(true); setCallStatus("Preparing protected call…");
  try{
   const created=await jsonFetch("/tasks",{method:"POST",body:JSON.stringify({intent:request,recipient:"Live recipient",contract:CONTRACT})});
   ...
   const started=await jsonFetch("/calls/calle/start",{method:"POST",body:JSON.stringify({task_id:created.task_id,phone,region:"GB",locale:"en-GB"})});
   setCallStatus("CALL-E job accepted. Waiting for phone outcome…");
   const deadline=Date.now()+8*60*1000;
   while(Date.now()<deadline){
    await sleep(3000);
    const job=await jsonFetch(`/calls/calle/status/${started.job_id}`);
    setCallStatus(`CALL-E: ${job.status}`);
    if(job.status==="COMPLETED"){setLiveResult(job.result);setStep(4);return}
    if(job.status==="FAILED")throw new Error(job.error||"CALL-E call failed.");
   }
   throw new Error("The call is still pending. Do not retry yet; check the provider status before creating another call.");
  }catch(e){setError(e.message||"Live CALL-E request failed.")}finally{setBusy(false)}
 }

 return <div className="shell"><header><div className="brand"><ShieldCheck size={22}/> Future Call AI</div><Badge kind={calle.configured?"ready":"muted"}>{calle.configured?"CALL-E READY":"NOT CONFIGURED"}</Badge></header>
 <div className="steps">{["Request","Contract","Guardian","Call","Result"].map((x,i)=><div className={`step ${i===step?"active":i<step?"done":""}`} key={x}><span>{i+1}</span>{x}</div>)}</div>
 <main>
 ...
 {mode==="simulated"?<><label>Simulated scenario</label><select value={scenario} onChange={e=>setScenario(e.target.value)}>{Object.entries(simulationScenarios).map(([k,v])=><option value={k} key={k}>{v.label}</option>)}</select><div className="notice">SIMULATED — no real phone call is placed.</div></>:<><label>Destination phone</label><input className="text-input" value={phone} onChange={e=>setPhone(e.target.value)} placeholder="+447700900123" inputMode="tel"/><div className="notice">LIVE CALL-E — this can place a real phone call. Server status: {calle.configured?"READY":"NOT CONFIGURED"}.</div></>}
 {callStatus&&busy&&<div className="notice">{callStatus}</div>}{error&&<div className="error">{error}</div>}<button disabled={!consent||busy} onClick={authorizeAndRun}>{busy?"Calling…":mode==="live"?"Authorize real CALL-E call":"Authorize simulated call"}<PhoneCall size={18}/></button></>}
