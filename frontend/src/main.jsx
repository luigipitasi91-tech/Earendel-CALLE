...
const CAPTURED_LIVE_PROOF={captured:true,provider_state:"completed",provider_task_completed:true,goal_state:"VERIFIED_SUCCESS",verified:["appointment","fee"],missing:[],contradicted:[],guardian:"ENFORCED",provider_evidence:["A live recipient answered and engaged with the rescheduling request.","The recipient confirmed the requested Friday afternoon move.","The recipient confirmed there would be no extra charge."],call_id:"call_4fEkmGdrngWlQz4uT7H_yg"};
...
function App(){
 const [step,setStep]=useState(0),[consent,setConsent]=useState(false),[creditConsent,setCreditConsent]=useState(false),[scenario,setScenario]=useState("verified"),[rating,setRating]=useState(0),[mode,setMode]=useState("simulated"),[phone,setPhone]=useState(""),[routeIndex,setRouteIndex]=useState(0);
 ...
 function showCapturedProof(){setMode("live");setLiveResult(CAPTURED_LIVE_PROOF);setError("");setCallStatus("");setStep(4)}
 ...
 async function authorizeAndRun(){
  ...
  if(!activeJobId&&!creditConsent){setError("Confirm that you understand this will spend CALL-E credits before starting a new live call.");return}
 ...
 {step===0&&<><p className="eyebrow">TRUSTED TASK COMPLETION</p><h1>Say what you need.<br/>We prove what happened.</h1><p className="lede">Future Call AI turns a phone task into a controlled goal, protects your permissions, and only marks success when the evidence supports it.</p><label>Your request</label><textarea value={request} onChange={e=>setRequest(e.target.value)}/><button onClick={()=>setStep(1)}>Compile Goal Contract <ChevronRight size={18}/></button><button className="secondary" onClick={showCapturedProof}>View captured live E2E proof — no call</button><div className="notice">Read-only production evidence from 13 Sep 2026. Viewing it uses no CALL-E credits.</div></>}
 ...
 {step===4&&<><p className="eyebrow">EVIDENCE & VERIFY</p><h2>What actually happened?</h2>{liveResult?.captured&&<div className="notice">CAPTURED PRODUCTION E2E — read-only evidence from 13 Sep 2026. No call was placed to display this screen.</div>}<div className={`verdict ${stateClass}`}>{displayedResult.result==="VERIFIED_SUCCESS"?<CheckCircle2/>:displayedResult.result==="FAILED"?<XCircle/>:<HelpCircle/>}<strong>{displayedResult.result}</strong><Badge>{mode==="live"?(liveResult?.captured?"CAPTURED LIVE E2E":"LIVE CALL-E"):"SIMULATED"}</Badge></div>{displayedResult.evidence.map(([title,ev],i)=><div className="evidence" key={i}><div className="e-title">{ev?"✓":"?"} {title}</div><div className="quote">{ev?`“${ev}”`:"No explicit supporting evidence found."}</div></div>)}{mode==="live"&&liveResult&&<div className="provider-card"><b>Provider state:</b> {String(liveResult.provider_state||"unknown")}<br/><b>Provider task_completed:</b> {String(liveResult.provider_task_completed)}<br/><b>Guardian:</b> {String(liveResult.guardian||"ENFORCED")}<br/><b>Earendel goal state:</b> {liveResult.goal_state}<br/><b>Provider call id:</b> {String(liveResult.call_id||"not exposed")}</div>}<div className="principle">CALL COMPLETED ≠ TASK COMPLETED ≠ VERIFIED SUCCESS</div><div className="rating"><p>How was your experience?</p>{[1,2,3,4,5].map(n=><Star key={n} onClick={()=>setRating(n)} fill={n<=rating?"currentColor":"none"}/>)}</div><button className="secondary" onClick={()=>{setStep(0);setConsent(false);setCreditConsent(false);setRating(0);setLiveResult(null);setError("");setCallStatus("");setMode("simulated")}}>Start a new request</button></>}
 </main></div>
}
